...
log.basicConfig(filename="genetic_algorithms_with_extreme_fitness.txt", filemode="w",
...
def evaluate_population(population, generation: int):
    factor = generation//5
    factor = min(factor, 8)
    return [max((run_iteration(individual, numeps)[0]-factor), 0)**2.5 for individual in population]
