...
def choose_actions(V):
    value = []

    for state in V:  # on policy
        value.append(random.choices(range(0, 4), weights=state, k=1)[0])
...
for process in range(0, 1000):
    ...
    lr = max(lr*0.995,0.001)
    for episodes in range(numeps):
        move = choose_actions(V)
    ...
    temp_reward, temp_wait, temp_trips = run_iteration(
        [i+offset for i in greedy(V)], 100)
    ...
    print(log_to_print)
    with open("monte_carlo_on_policy.txt", "a") as file:
